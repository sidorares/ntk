...
/**
 * The pattern a family and face are matched with — one spelling for both
 * `match` and `fallbackFor`, since a source caches its lists by pattern and
 * the two ask about the same face.
 */
function patternOf(family, weight, italic) {
  return {
    family: familiesOf(family).join(','),
    weight,
...
export default class FontManager {
  ...
  match(family = 'sans-serif', opts = {}) {
    ...
    const families = familiesOf(family);
  ...
  fallbackFor(codepoint, family = 'sans-serif', opts = {}) {
    ...
    if (!found) {
      ...
      let candidates;
      try {
        // The pattern `match` asks for this family and face, so the list the
        // match or a prewarm already fetched answers: asked with the span's
