 ...
 * whichever call made it first. A glyph asked for by id alone — its extents,
 * its outline, or a component of a composite, which fontkit makes from inside
 * the composite's path with no characters — is made with none, and every
 * later shaping of that letter was handed the same object: a run read `e` as
 * no characters at all, so every caret, selection edge and hit test after it
 * on the line was a character over. Two code points that share a glyph (a
 * space and a no-break space) had the same trouble the other way round.
 *
 * Asked with characters the cached glyph does not carry, the face answers a
 * view of it that does — its prototype is the cached glyph, so the outline,
 * the metrics and every cache on it stay shared — one view per glyph and
 * characters. Installed once per fontkit font, on the instance, which is
 * where fontkit's own shaping looks it up.
 */
function answerWithCharacters(fk) {
  if (fk.__ntkCharacters) return;
  const getGlyph = fk.getGlyph;
  const views = new Map();
  fk.getGlyph = function (id, characters = []) {
    const glyph = getGlyph.call(this, id, characters);
    if (!glyph || characters.length === 0) return glyph;
    const had = glyph.codePoints;
    if (had.length === characters.length && had.every((cp, i) => cp === characters[i])) {
      return glyph;
    }
    const key = `${id}:${characters.join(',')}`;
    let view = views.get(key);
    if (view === undefined) {
      view = Object.create(glyph, {
        codePoints: { value: characters.slice(), enumerable: true }
      });
      views.set(key, view);
    }
    return view;
...
export default class Font {
  constructor(fkFont, path, postscriptName) {
    ...
    // What the face's tables say, read once. fontkit's getters walk to the
    // table on every read, and a text layout asks per run and per word: in
    // a long document they were most of a relayout (see `metrics`).
    this._upem = 0;
    this._vertical = null;
    this._space = -1;
  }
  ...
  scale(size) {
    return size / (this._upem ||= this.fk.unitsPerEm);
  }
   ...
   *
   * A new object every call, which a caller may keep or change; the numbers
   * behind it are the face's, read from its tables once.
   */
  metrics(size) {
    ...
    const descent = v.descent * s;
    const lineGap = v.lineGap * s;
    return {
      ascent,
      descent,
      lineGap,
      lineHeight: ascent + descent + lineGap,
      capHeight: v.capHeight * s,
      xHeight: v.xHeight * s
    };
  }

  /**
   * The advance of U+0020 at `size` — what a run of trailing spaces is
   * worth to a line. Looked up once: it was a cmap lookup for every word a
   * layout ended with a space, which in a paragraph is every word.
   */
  spaceAdvance(size) {
    if (this._space < 0) this._space = this.fk.glyphForCodePoint(0x20).advanceWidth;
