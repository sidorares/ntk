...
// steps per pixel a run origin is snapped to before its glyphs round (see
// positionGlyphs); a power of two, so the snap and the split are exact
...
export function positionGlyphs(positioned) {
  ...
  for (const { run, x, y } of positioned) {
    const sx = Math.round(x * ORIGIN_STEPS);
    const sy = Math.round(y * ORIGIN_STEPS);
    const ox = Math.floor(sx / ORIGIN_STEPS);
    const oy = Math.floor(sy / ORIGIN_STEPS);
    const fy = (sy - oy * ORIGIN_STEPS) / ORIGIN_STEPS;
    let cursor = (sx - ox * ORIGIN_STEPS) / ORIGIN_STEPS;
