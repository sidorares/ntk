...
RC.fillText = function(text, x, y) {
  ...
  Render.CompositeGlyphs32(3, this._background.id, this.picture.id, 0, this._glyphSet.id, 0, 0, [ [x, y, glyphs ] ] );
};
...
CanvasGradient.prototype.__defineGetter__('id', function() {
  ...
  this._id = X.AllocID();
  console.log('Instantiting gradient!!!', this.type);
  switch (this.type) {
  case 'linear':
...
function parseColor(str) {
  var knownColors = require('./named_colors.js');
  if (typeof str == 'string') {
    var c = knownColors[str.toLowerCase()];
    if (c)
      return c;
