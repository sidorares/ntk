...
RC._rasterizePath = function(picture) {
  ...
  const loops = this._currentPath.map(loop => loop.map(pt => [pt.x, pt.y]));
  var pslg = poly2pslg(loops, { nested: true });
  var cdt2d = require('cdt2d');
  const triangles = cdt2d(pslg.points, pslg.edges, { exterior: false, nested: true });
  const triPoints = [];
...
RC.drawImage = function(image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight) {
  ...
  if (image.constructor == RenderingContext2d) {
    ...
    var pixmap = new Pixmap(this.window.app, {
      depth: 32,
      width: sWidth,
      height: sHeight
    });
