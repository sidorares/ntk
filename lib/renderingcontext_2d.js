...
module.exports = function(window) {
  if (window._ctx2d) {
    return window._ctx2d;
  }
  const ctx = new RenderingContext2d(window);
  window._ctx2d = ctx;
  return ctx;
};
...
RC.strokeRect = function(x, y, w, h) {
  // TODO: use specialised version instead of generic poly extrusion
  this.beginPath();
  this.moveTo(x, y);
  this.lineTo(x + w, y);
  this.lineTo(x + w, y + w);
...
function bcurveStep(x0, y0, x1, y1, x2, y2, x, y) {
  ...
  var dt = 1 / Math.ceil(Math.max(maxX - minX, maxY - minY));
  if (dt > 0.2) dt = 0.2;
  return dt;
}
...
RC.quadraticCurveTo = function(x1, y1, x, y) {
  var lastPt = this._currentPoly[this._currentPoly.length - 1];
  var quadratic = require('bezier').prepare(3);
  var x = [lastPt.x, x1, x];
  var y = [lastPt.y, y1, y];
  var step = bcurveStep(x[0], y[0], x[1], y[1], x[1], y[1], x[2], y[2]);
  for (var t = 0; t <= 1; t += step) {
    this.lineTo(quadratic(x, t), quadratic(y, t));
  }
};
...
function xCoord_of_segment_at_Y(inSegment, inCrossYPt) {
  ...
  return (xCoord =
    inSegment.vFrom.x +
    ((inSegment.vTo.x - inSegment.vFrom.x) *
      (inCrossYPt.y - inSegment.vFrom.y)) /
      (inSegment.vTo.y - inSegment.vFrom.y));
}

RC._rasterizePath2 = function(picture) {
  var poly2pslg = require('poly-to-pslg');
  var simplify = require('simplify-path');
  const loops = this._currentPath.map(loop =>
    simplify(loop.map(pt => [pt.x, pt.y]), 0.5)
  );

  var pslg = poly2pslg(loops, { nested: true });
  var cdt2d = require('cdt2d');
  const triangles = cdt2d(pslg.points, pslg.edges, {
    exterior: false,
    nested: true
  });
  const triPoints = [];
  triangles.forEach(tri => {
    for (var i = 0; i < 3; ++i) {
      triPoints.push(pslg.points[tri[i]][0]);
      triPoints.push(pslg.points[tri[i]][1]);
    }
  });
...
RC._rasterizeTrapFill = function() {
  var PNLTRI = require('pnltri');
  var max_width = 65535;
  var simplify = require('simplify-path');
  const path = this._currentPath.map(loop =>
    simplify(loop.map(pt => [pt.x, pt.y]), 0.2).map(pt => ({
      x: pt[0],
      y: pt[1]
    }))
  );
...
RC.fillTraps = function(trapSegments) {
  if (!this.fillMask) {
  ...
  this.Render.AddTraps(this.fillMask.id, 0, 0, trapSegments);
};
...
// TODO: split it into preparePicture() + _drawPicture() (that respects position/trsnform/mask/global blending)
RC.drawImage = function(
  image,
  sx,
  sy,
  sWidth,
  ...
  if (image.constructor == RenderingContext2d) {
    ...
    const maskId = this.clipMask ? this.clipMask.id : 0;
    this.Render.Composite(
      this.Render.PictOp.Over,
      image.picture.id,
      0, //maskId,
    ...
    var pixmap = this.window.createPixmap({
      depth: 32,
      width: sWidth,
      height: sHeight
    });
    var picture = new Picture(this.window.app, {
      drawable: pixmap,
