// XRender (RENDER) extension for node-x11's pure-JS X server.
//
// Registered via server.registerExtension('RENDER', createRenderExtension())
// — see lib/xserver/index.js for the node-side glue. The module is fully
// self-contained (no node builtins, nothing imported from x11) so a browser
// playground can register the identical extension against the browser build
// of the server.
//
// The wire decoder mirrors node-x11's client encoder (x11/lib/ext/render.js),
// which is the only client this server has to satisfy. Two deviations from
// stock renderproto follow from that:
//  - AddTraps carries SPANFIX trapezoids (top l/r/y, bottom l/r/y — what
//    ntk's trapezoidize emits), the same layout Xorg uses for AddTraps.
...
// core X protocol error codes (subset used here; values are fixed by the
// protocol so hardcoding keeps the module dependency-free)
const codes = {
  Value: 2,
  Match: 8,
  Drawable: 9,
...
// server.handleRequest turns thrown { isXError: true } objects into protocol
// errors with the right sequence number and major/minor opcodes
function xerror(code, badValue = 0) {
  return { isXError: true, code, badValue };
}

const FIXED = 65536; // 16.16 fixed point
const pad4 = (n) => (n + 3) & ~3;

// wire COLOR (4 x CARD16, premultiplied) -> [a, r, g, b] floats 0..255.
...
// ---------------------------------------------------------------------
// antialiased coverage accumulation, shared by AddTraps and Triangles.
//
  ...
  for (let Y = yStart; Y < yEnd; Y++) {
    ...
    for (let s = 0; s < SUB; s++) {
      ...
      for (let X = x0; X < x1; X++) {
        ...
        if (covered > 0) cov[rowBase + X] += covered * step;
      }
    }
  }
}
...
// offset; t outside the stop range clamps to the edge stops (pad), which is
// what canvas semantics (and ntk's CanvasGradient) expect.
...
/**
 * Build the RENDER extension definition for server.registerExtension.
 * One instance per server (init() captures per-server state).
 *
 * @param {object} [options] reserved
 */
export function createRenderExtension(options = {}) { // eslint-disable-line no-unused-vars
  // per-server state, filled by init()
  let firstError = 0;
  const formats = new Map(); // format id -> descriptor

  // picture format ids: arbitrary XIDs outside any client's resource range.
  // The descriptors are exactly what node-x11's require('render') handshake
  ...
  const badPicture = (xid) => xerror(firstError + ERR_PICTURE, xid);
  const badGlyphset = (xid) => xerror(firstError + ERR_GLYPHSET, xid);

  function getPicture(server, xid) {
    const res = server.resources.get(xid);
    if (!res || res.type !== 'picture') throw badPicture(xid);
    return res;
  }

  function getGlyphset(server, xid) {
    const res = server.resources.get(xid);
    if (!res || res.type !== 'glyphset') throw badGlyphset(xid);
    return res;
  }

  // pictures with backing storage (as opposed to solid/gradient sources)
  function targetRaster(pict) {
    if (!pict.drawable) throw xerror(codes.Match, pict.id);
    return pict.drawable.raster; // read per-op: window rasters are replaced on resize
  }

  function damage(server, pict) {
    if (pict.drawable && pict.drawable.type === 'window') server.damageWindow(pict.drawable);
  }
  ...
  function gradientParam(pict, x, y) {
    switch (pict.kind) {
      ...
      case 'radial': {
        ...
        if (Math.abs(a) < 1e-9) return b !== 0 ? c / (2 * b) : 0;
        const disc = b * b - a * c;
        if (disc < 0) return 0;
  ...
  // -------------------------------------------------------------------
  // the core composite loop, shared by Composite / FillRectangles /
  // Triangles / glyphs. `coverage(i, j)` (0..1) modulates the source like a
  ...
  function compositeSpan(op, sample, srcX, srcY, mask, dst, dstX, dstY, w, h, coverage) {
    ...
    for (let j = 0; j < h; j++) {
      ...
      for (let i = 0; i < w; i++) {
        ...
        if (!factors) throw xerror(firstError + ERR_PICTOP, op);
        const [fa, fb] = factors;
        writePx(
          raster, depth, X, Y,
          sa * fa + d[0] * fb,
  ...
  function createPicture(server, client, body) {
    ...
    if (!format) throw xerror(firstError + ERR_PICTFORMAT, body.readUInt32LE(8));
    if (format.depth !== drawable.depth) throw xerror(codes.Match, drawable.id);
  ...
  // Only repeat and componentAlpha affect rendering here; polyEdge/polyMode
  // (always-antialiased rasterization), clip and alpha-map values are
  // accepted and ignored — ntk never relies on them server-side.
  function applyPictureValues(pict, body, maskOff) {
    const mask = body.readUInt32LE(maskOff);
    let off = maskOff + 4;
    for (let bit = 0; bit < 13; bit++) {
      if (!(mask & (1 << bit))) continue;
      if (bit === 0) pict.repeat = body.readUInt8(off);
      else if (bit === 12) pict.componentAlpha = body.readUInt8(off);
      off += 4;
    }
  }
  ...
  function queryPictFormats(server, client) {
    ...
    // one subpixel entry. node-x11's handshake only walks the format list,
  ...
  function queryFilters(client) {
    ...
    let strBytes = 0;
    for (const n of names) strBytes += 1 + n.length;
  ...
  function composite(server, body) {
    ...
    const mask = maskId
      ...
      : null;
    compositeSpan(op, makeSampler(src), srcX, srcY, mask, dst, dstX, dstY, w, h, null);
    damage(server, dst);
  }
  ...
  function triangles(server, body) {
    const op = body.readUInt8(0);
    const src = getPicture(server, body.readUInt32LE(4));
    ...
    let maxY = -Infinity;
    const tris = [];
    for (let off = 20; off + 24 <= body.length; off += 24) {
      const t = [
        body.readInt32LE(off) / FIXED,
        body.readInt32LE(off + 4) / FIXED,
        body.readInt32LE(off + 8) / FIXED,
        body.readInt32LE(off + 12) / FIXED,
        body.readInt32LE(off + 16) / FIXED,
        body.readInt32LE(off + 20) / FIXED
      ];
      tris.push(t);
      for (let k = 0; k < 6; k += 2) {
    ...
    for (const t of tris) {
      // sort the three vertices by y and split at the middle one: each half
      // is a trapezoid with linear left/right edges
      const v = [
        [t[0] - ex0, t[1] - ey0],
        [t[2] - ex0, t[3] - ey0],
        [t[4] - ex0, t[5] - ey0]
      ...
      const [v0, v1, v2] = v;
      if (v2[1] <= v0[1]) continue; // degenerate
      const xm = v0[0] + ((v2[0] - v0[0]) * (v1[1] - v0[1])) / (v2[1] - v0[1]);
      accumulateTrap(cov, ew, eh, v0[0], v0[0], v0[1], v1[0], xm, v1[1]);
      accumulateTrap(cov, ew, eh, v1[0], xm, v1[1], v2[0], v2[0], v2[1]);
    }

    // per Xorg, the source aligns srcX/srcY with the first point's integer
    // position (a no-op for ntk's repeating solid sources with srcX/srcY 0)
    const xDst = Math.floor(tris[0][0]);
    const yDst = Math.floor(tris[0][1]);
    compositeSpan(
      op, makeSampler(src),
      srcX + ex0 - xDst, srcY + ey0 - yDst,
  ...
  function addTraps(server, body) {
    ...
    // the saturating add: traps produced by ntk's slab decomposition share
  ...
  function setPictureFilter(server, body) {
    ...
    if (!known.includes(name)) throw xerror(codes.Value, 0);
    const params = [];
  ...
  function addGlyphs(server, body) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (imgOff + len > body.length) throw xerror(codes.Length, len);
  ...
  function compositeGlyphs(server, body, idBytes) {
    ...
    const src = getPicture(server, body.readUInt32LE(4));
    const dst = getPicture(server, body.readUInt32LE(8));
    // maskFormat (offset 12): ntk always passes None; each glyph is
    // composited individually, which is identical for the Over/opaque-src
    // draws ntk issues and avoids a second coverage pass
    ...
    while (off + 8 <= body.length) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        if (!g) throw xerror(firstError + ERR_GLYPH, id);
        if (first) {
          first = false;
          srcOffX = srcX - penX;
          srcOffY = srcY - penY;
        ...
        for (let row = 0; row < g.height; row++) {
          ...
          for (let col = 0; col < g.width; col++) {
            ...
            if (c === 0 && (op === 3 || op === 12)) continue; // Over/Add: no-op
            const f = c / 255;
  ...
  return {
    ...
    init(server, extInfo) {
      firstError = extInfo.firstError;
      for (const f of FORMAT_LIST) formats.set(f.id, f);
    },
    ...
    handleRequest(server, client, minor, body) {
      switch (minor) {
        ...
        case 2: // QueryPictIndexValues: no indexed formats on this server
          throw xerror(codes.Match, body.readUInt32LE(0));
        ...
        case 11: // Triangles
          return triangles(server, body);
        case 17: { // CreateGlyphSet
          const gsid = body.readUInt32LE(0);
          const format = formats.get(body.readUInt32LE(4));
          if (!format) throw xerror(firstError + ERR_PICTFORMAT, body.readUInt32LE(4));
          if (format.depth !== 8) throw xerror(codes.Match, format.id); // a8 only
        ...
        default:
          // Trapezoids(10), TriStrip/TriFan(12/13), ReferenceGlyphSet(18),
          // AddGlyphsFromPicture(21), cursors(27/31), clip rects(6),
          // SetPictureClipRectangles-dependent paths: unused by ntk
          throw xerror(codes.Implementation, minor);
      }
    }
  };
}

export default createRenderExtension;
