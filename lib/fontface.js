...
FontFace.prototype.upload = function(size, hdpi, vdpi) {
  ...
  while (gindex.gindex !== 0) {
    ...
    if (gindex.gindex > maxIndex)
      maxIndex = gindex.gindex;
  ...
  // TODO: better way to handle whitespace
  var g = {
    id: maxIndex+1,
    charcode: 32.
  };
  ...
  g.offX = 0;
  g.offY = 0;
  g.offX = this.charcode2glyphObj['n'.charCodeAt(0)].offX ;
  this.charcode2glyph[32] = g.id;
  this.charcode2glyphObj[32] = g;
  glyphs.push(g);

  var gs = new GlyphSet(this.app);
  //var em = this.charcode2glyphObj['m'.charCodeAt(0)];
  gs.addGlyphs(glyphs);
  this._gs[size] = gs;
  return gs;
};
