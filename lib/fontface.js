...
FontFace.prototype.loadFont = function(path) {
  ...
  this._fontkitFont = fontkit.openSync(path);
  this.charcode2glyph = [];
  this.charcode2glyphObj = [];
};
...
FontFace.prototype.upload = function(size, hdpi, vdpi) {
  if (this._gs[size]) return this._gs[size];
  ...
  while (gindex.gindex !== 0) {
    ...
    glyphs.push(g);
    this.charcode2glyphObj[charcode] = g;
  }
...
FontFace.prototype.uploadGlyphs = function(size, hdpi, vdpi, ids) {
  if (!this._gs[size]) {
    this._gs[size] = new GlyphSet(this.app);
  }
  var face = this._face;
  var gindex = {};
  var charcode;
  ...
  charcode = ft2.Get_First_Char(face, gindex);
  ids.forEach(glyph => {
    ft2.Load_Glyph(face, glyph, ft2.LOAD_RENDER);
