...
export default class App {
  ...
  constructor(display, options = {}) {
    ...
    // resolved extension is kept alongside the promise because a region clip
    // has to be installed synchronously, in request order with the drawing
    // around it — see RenderingContext2d#clipRegion.
    this._fixes = null;
    this._fixesPromise = null;
   ...
   */
  fixes() {
    if (this._fixesPromise) return this._fixesPromise;
    this._fixesPromise = new Promise((resolve, reject) => {
      this.X.require('fixes', (err, ext) => {
        if (err || !ext) {
          this._fixesPromise = null; // let a later call try again
          reject(noXFixesError(err));
          return;
        }
        this._fixes = ext;
        resolve(ext);
