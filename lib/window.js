...
function Window(app, args) {
  ...
  const _reflow = () => {
    ...
    if (true) {
      ...
      this.backBuffer.fillStyle = 'blue';
      this.backBuffer.fillRect(0, 0, 1000, 1000);
      this.document.render(this.backBuffer);
      const frontBufferer = this.getContext('2d');
      frontBufferer.drawImage(this.backBuffer);
    } else {
      const frontBufferer = this.getContext('2d');
      this.document.render(frontBufferer);
    }
  };
...
function dbg1(node) {
  console.log(`=1 ${node.name} ${node.style} (${node._viewportLeft} ${node._viewportTop})`);
}

function dbg(node) {
  if (node.name) {
    console.log(`= ${node.name} ${node.style} (${node._viewportLeft} ${node._viewportTop})`);
  }
...
Window.prototype.__defineGetter__('document', function() {
  if (!this._document) {
    const document = new Document({ window: this });
