...
function Window(app, args) {
  ...
  if (this.constructor !== Window) return new Window(app, args);
  ...
  this.app = app;
  this._rafQueue = [];
  this.document = new Document({ window: this });
  ...
  if (!args.parent) parentId = display.screen[0].root;
  else {
  ...
  const _reflow = () => {
    const rootLayout = this.document.body._layoutNode;
    const { width, height } = rootLayout.getComputedLayout();

    if (width === this.width && height === this.height) {
      return;
    }

    rootLayout.setWidth(self.width);
    rootLayout.setHeight(self.height);
    rootLayout.calculateLayout();
    console.log(`Reflow + update: ${self.width} ${self.height} ${+new Date()}`);

    if (true) {
      const backBufferPx = this.createPixmap();
      this.backBuffer = backBufferPx.getContext('2d');
      this._renderDocument(this.backBuffer);
      const frontBufferer = this.getContext('2d');
      frontBufferer.drawImage(this.backBuffer);
    } else {
      const frontBufferer = this.getContext('2d');
      this._renderDocument(frontBufferer);
    }

    //console.log('_reflow draw back buffer on top of front');

    //this.document.body.render(frontBufferer);
  };

  const _reflowThrottled = throttle(_reflow, 16);
  //this.reflow = () => this.queryPointer(_reflowThrottled);
  this.reflow = () => _reflowThrottled();
  ...
  this.on('expose', ev => {
    return;
    // TODO: add rects to damage area and throttle render
    if (!this.backBuffer) {
      console.log('No back buffer yet, skipping');
      return;
    }
    //this.document.body.render(this.backBuffer, ev);

    const frontBufferer = this.getContext('2d');
    console.log('"expose" draw back buffer on top of front');
    frontBufferer.drawImage(this.backBuffer);
  });
...
function dbg1(node) {
  console.log(
    `=1 ${node.name} ${node.style} (${node._viewportLeft} ${node._viewportTop})`
  );
}
...
function dbg(node) {
  if (node.name) {
    console.log(
      `= ${node.name} ${node.style} (${node._viewportLeft} ${
        node._viewportTop
      })`
    );
  }
  ...
  let n = node;
  while ((n = n.parent)) {
    console.log(`-> ${n.name} (${n._viewportLeft} ${n._viewportTop})`);
  }
}

// TODO: make async / interruptable
Window.prototype._renderDocument = function(backBuffer) {
  this.document.body.render(backBuffer);
  let queue = this.document.body.children.slice();
  // TODO: body scroll position? Or make body absolutely positioned element and handle via layout?
  this.document.body._viewportLeft = 0;
  this.document.body._viewportTop = 0;
  let x = 0;
  let y = 0;
  while (queue.length) {
    ...
    const {
      ...
      left,
      top
    } = currentElement._layoutNode.getComputedLayout();
    currentElement._viewportLeft = currentElement.parent._viewportLeft + left;
    currentElement._viewportTop = currentElement.parent._viewportTop + top;
    const ctx = this.createPixmap({ width, height }).getContext('2d');
...
Window.prototype._pumpRafQueue = function() {
  this._rafActive = true;
  // TODO: better way to check status of how many commands processed by server?
  // AFAIK we get this response at the end of the queue
  this.queryPointer(() => {
    const task = this._rafQueue.shift();
    if (task) {
      setTimeout(() => {
        console.log('Pump!', +new Date());
        task();
        this._pumpRafQueue();
      }, 10);
    } else {
...
Window.prototype.requestAnimationFrame = function(task) {
  this._rafQueue.push(task);
  if (!this._rafActive) {
    this._pumpRafQueue();
  }
};
...
const changeAttributesPropNames =
  'backgroundPixmap backgroundPixel borderPixmap borderPixel bitGravity winGravity backingStore backingPlanes backingPixel overrideRedirect saveUnder eventMask doNotPropagateMask colormap cursor';
...
Window.prototype.setTitle = function(title) {
  this.X.ChangeProperty(
    0,
    this.id,
    this.display.client.atoms.WM_NAME,
    this.display.client.atoms.STRING,
    8,
    title
  );
  return this;
...
Window.prototype.queryTree = function(callback) {
  ...
  this.X.QueryTree(this.id, function(err, tree) {
    ...
    var children = tree.children.map(function(id) {
      return new Window(app, { id: id });
    });
...
Window.prototype.setActions = function() {
  ...
  var X = this.X;
  var wid = this.id;
  X.InternAtom(false, '_NET_WM_ACTION_CLOSE', function(
    err,
